  ...
  public void execute() {
    drivetrain.setControl(
                drive45.withTargetDirection(targetAngle) // Lock the robot at 45 degrees
                    .withVelocityX(-Robot.getInstance().gamepad.getLeftY() * MaxSpeed * 0.6) // Drive forward with negative Y (forward)
                    .withVelocityY(-Robot.getInstance().gamepad.getLeftX() * MaxSpeed * 0.6) // Drive left with negative X (left)
            );
